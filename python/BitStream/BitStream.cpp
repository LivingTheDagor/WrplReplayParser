...
void PyBitStream::include(py::module_ &m) {
  ...
  py::class_<BitStream>(m, "BitStream")
    ...
    .def(
      "ReadBytes",
      [](BitStream &bs, uint32_t bytes) -> std::optional<py::bytes> { return read_bits(bs, BYTES_TO_BITS(bytes)); },
      py::arg("bytes"))
    .def(
      "ReadBits", [](BitStream &bs, uint32_t bits) -> std::optional<py::bytes> { return read_bits(bs, bits); },
      py::arg("bits"))
    .def("ReadUInt8",
         [](BitStream &bs) -> std::optional<uint8_t> {
           uint8_t v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadUInt16",
         [](BitStream &bs) -> std::optional<uint16_t> {
           uint16_t v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadUInt32",
         [](BitStream &bs) -> std::optional<uint32_t> {
           uint32_t v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadUInt64",
         [](BitStream &bs) -> std::optional<uint64_t> {
           uint64_t v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadInt8",
         [](BitStream &bs) -> std::optional<int8_t> {
           int8_t v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadInt16",
         [](BitStream &bs) -> std::optional<int16_t> {
           int16_t v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadInt",
         [](BitStream &bs) -> std::optional<int32_t> {
           int32_t v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadInt64",
         [](BitStream &bs) -> std::optional<int64_t> {
           int64_t v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadFloat",
         [](BitStream &bs) -> std::optional<float> {
           float v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadDouble",
         [](BitStream &bs) -> std::optional<double> {
           double v;
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadString",
         [](BitStream &bs) -> std::optional<std::string> {
           std::string v{};
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadBool",
         [](BitStream &bs) -> std::optional<bool> {
           bool v{};
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def("ReadBS",
         [](BitStream &bs) -> std::optional<BitStream> {
           BitStream v{};
           if (bs.Read(v)) {
             return {v};
           }
    ...
    .def(
      "readBitsInt",
      [](BitStream &bs, uint32_t bits) -> std::optional<uint64_t> {
        if (bits > 64) {
    ...
    .def("GetData", [](BitStream &bs) {
      return py::memoryview::from_memory(bs.GetData(), (ssize_t) bs.GetNumberOfBytesUsed(), true);
    });
  m.def(
    "lz4_decompress",
    [](py::bytes &in_data, size_t &out_len) -> py::bytes {
      auto spn = bytes_to_span(in_data);
      std::string payload{};
      payload.resize(out_len);
      int readedSize = LZ4_decompress_safe(reinterpret_cast<const char *>(spn.data()), payload.data(), (int) spn.size(),
                                           (int) payload.size());
      if (readedSize < 0) {
  ...
  m.def("BYTES_TO_BITS", [](BitSize_t in) { return BYTES_TO_BITS(in); });
  m.def("BITS_TO_BYTES", [](BitSize_t in) { return BITS_TO_BYTES(in); });


  py::class_<IdFieldSerializer255>(m, "IdFieldSerializer255")
    .def(py::init<>())
    .def(
      "readFieldsSizeAndCount",
      [](IdFieldSerializer255 &serializer255, BitStream &bs) -> py::tuple {
        BitSize_t end;
