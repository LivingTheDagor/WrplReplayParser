...
struct task_group_pool {
    task_group_pool()
            : pool_size(tbb::this_task_arena::max_concurrency()),
              task_submitters(new tbb::task_group[pool_size]) {}
    ...
    tbb::task_group& operator[](std::size_t idx) {
        return task_submitters[idx];
    }
...
public:
    ...
    base_task(const base_task& t)
            : m_type(t.m_type),
              m_parent(t.m_parent),
              m_child_counter(t.m_child_counter.load()) {}
...
protected:
    enum class task_type { stack_based, allocated, recycled };
...
public:
    root_task(tbb::task_group& tg)
            : m_tg(tg),
              m_callback(
                  m_tg.defer([] { /* Create empty callback to preserve reference for wait. */ })) {
        add_child_reference();
