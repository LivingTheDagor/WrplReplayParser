    ...
    task_emulation::base_task* compute_impl() {
        ...
        if (n < cutoff) {
            *x = serial_fib_1(n);
            ...
            task_emulation::run_task(
                this->allocate_child_and_increment<single_fib_task>(n - 1, &x_l));
...
int fibonacci_single_task(int n) {
    int sum{};
    tbb::task_group tg;
    task_emulation::run_and_wait(
        tg, task_emulation::allocate_root_task<single_fib_task>(/* for root task = */ tg, n, &sum));
    return sum;
}
