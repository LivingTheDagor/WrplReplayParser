...
namespace net
{
  ...
  BitStream &DeltaComp::writeNextData(History &history)
  {
    ...
    for (History::CacheEntry &c : history.compressedCache)
      c.clear();
    BitStream &bs = history.baseHist[history.curPacketNo & (history.baseHist.size() - 1u)];
    bs.SetWriteOffset(0);
    return bs;
  }

  const BitStream &DeltaComp::getData(const History &history)
  {
  ...
  static void write_aligned_bytes(BitStream &dst, const uint8_t *data, uint32_t size_bytes)
  {
#if 0
  ...
  void DeltaComp::writeDelta(BitStream &out_data, History &history, State &state, bool reliable_channel, bool use_cache)
  {
    ...
    BitStream &uncompressedData = history.baseHist[nextPacketIdx];
...
// check if idx1 goes before idx2 for N bit unsigned integers, considering wraparound
  ...
  ok &= data.Read(srvHash);
    ...
    BitStream compressedDelta(data.GetData() + BITS_TO_BYTES(data.GetReadOffset()), compressedSize, false);
    ...
    {
      BitStream &to = history->baseHist[nextPacketIdx];
      to.SetWriteOffset(0);
  ...
  void DeltaComp::writeConfirm(BitStream &bs, const ReadResult &result)
  {
    G_ASSERT(result.ok);
    bs.WriteBits(reinterpret_cast<const uint8_t *>(&result.confirmPacketNo), indexBits);
  }

  bool DeltaComp::readConfirm(const BitStream &bs, const History *history, State *state)
  {
    bool ok = true;
  ...
  void DeltaComp::writeReplaySnapshot(BitStream &bs, const History &history)
  {
    bs.Write(history.isEnabled);
    if (!history.isEnabled)
      return;
    bs.WriteBits(reinterpret_cast<const uint8_t *>(&history.curPacketNo), indexBits);
    for (const BitStream &base : history.baseHist)
      bs.Write(base);
  }

  bool DeltaComp::readReplaySnapshot(const BitStream &bs, History *history)
  {
    bool ok = true;
    ...
    if (history)
    {
      history->baseHist = std::move(h.baseHist);
