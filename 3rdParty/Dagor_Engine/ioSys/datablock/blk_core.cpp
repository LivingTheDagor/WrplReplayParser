...
void DataBlock::createData() {
    G_ASSERT(!data);
    if (shared)
        data = new(shared->allocateData()) DataBlockOwned;
}
...
// I have to do this because gaijin allocator is all malloc under the hood, but stdlib isn't :|
DataBlock::DataBlock() : shared(new(malloc(sizeof(DataBlockShared))) DataBlockShared) { nameIdAndFlags |= IS_TOPMOST; }
DataBlock::DataBlock(const char *filename) : DataBlock() { load(filename); }
...
DataBlock::DataBlock(DataBlockShared *s, int nid, uint16_t pcnt, uint16_t bcnt, uint32_t fBlock,
                     uint32_t ofs_) : shared(s), nameIdAndFlags(nid + 1), paramsCount(pcnt), blocksCount(bcnt),
                                      firstBlockId(fBlock), ofs(ofs_) {
}
...
const char *DataBlock::getParamData(const Param &p) const {
    ...
    return dblk::get_type_size(p.type) <= INPLACE_PARAM_SIZE
               ? (const char *) (&pv)
               : (rw ? rwDataAt(getUsedSize() + pv) : roDataAt(pv));
}
...
int DataBlock::findBlock(int nid, int after) const {
    return isBlocksOwned() ? findBlockRW(nid, after) : findBlockRO(nid, after);
}
...
DataBlock *DataBlock::getBlockByName(int name_id, int start_after, bool expect_single) {
    ...
    (void) (expect_single);
#endif
    return isBlocksOwned()
               ? getBlockRW(findBlockRW(name_id, start_after))
               : getBlockRO(findBlockRO(name_id, start_after));
}

const DataBlock *DataBlock::getBlockByName(int name_id, int start_after, bool expect_single) const {
    return const_cast<const DataBlock *>(const_cast<DataBlock *>(this)->getBlockByName(
        name_id, start_after, expect_single));
}
...
const char *DataBlock::getName(int name_id) const {
    return name_id >= 0 && shared ? shared->getName(name_id) : nullptr;
}
...
struct DbUtils {
    __forceinline static int find(int name_id, const DataBlock::Param *__restrict s,
...
int DataBlock::findParam(int name_id) const {
    return isOwned() ? findParam < true > (name_id) : findParam < false > (name_id);
}
