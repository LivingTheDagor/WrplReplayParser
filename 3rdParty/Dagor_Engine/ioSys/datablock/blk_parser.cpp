...
static std::string mk_sub_str(const char *sb, const char *se) {
  std::string s{sb, static_cast<std::string::size_type>(se - sb)};
  return s;
}
...
public:
  virtual bool resolveIncludeFile(std::string &inout_fname) {
    if (!inout_fname.empty() && inout_fname[0] == '#') {
      inout_fname = fmt::format("{}/{}", root, &inout_fname[1]);
      return true;
    }
    if (!inout_fname.empty() && inout_fname[0] == '%')
      return true;
...
public:
  ...
  std::vector<int> includeStackPrevCurLine;
  ...
  std::vector<PendingComment> pendCmnt;
  DataBlock::IFileNotify *fnotify;

  DataBlockParser(std::vector<char> &buf, const char *fn, bool robust_parsing, DataBlock::IFileNotify *fnot) :
    buffer(buf),
    text(&buf[0]),
    curp(&buf[0]),
...
private:
  ...
  void flushPendingComments(DataBlock &blk, bool to_params) {
    for (auto &c: pendCmnt) {
      ...
      if (to_params)
        blk.addStr(nm, mk_sub_str(c.ss, c.se).c_str());
      else
        blkRef->addNewBlock(nm)->addStr(nm, mk_sub_str(c.ss, c.se).c_str());
    }
    pendCmnt.clear();
  }
};
        ...
        if (nc == '/' && parse_comments) {
          ...
          if (DAGOR_UNLIKELY(DataBlock::parseCommentsAsParams)) {
            if (wasNewlineAfterStatement || lastStatement == -1)
              ...
              blkRef->addStr(COMMENT_POST_CPP, mk_sub_str(cpp_comment_start, curp - 1).c_str());
            else
              blkRef->addNewBlock(COMMENT_POST_CPP)
                ->addStr(COMMENT_POST_CPP, mk_sub_str(cpp_comment_start, curp - 1).c_str());
          }
          continue;
          ...
          if (DAGOR_UNLIKELY(DataBlock::parseCommentsAsParams)) {
            if (wasNewlineAfterStatement || lastStatement == -1)
              ...
              blkRef->addStr(COMMENT_POST_C, mk_sub_str(c_comment_start, curp - 2).c_str());
            else
              blkRef->addNewBlock(COMMENT_POST_C)
                ->addStr(COMMENT_POST_C, mk_sub_str(c_comment_start, curp - 2).c_str());
          }
          continue;
        } else
          --curp;
...
struct TempString {
  ...
  char buf[SSO_SIZE];
  uint16_t len = 0;
  std::string str; // can be unioned with buf
  TempString() { buf[0] = 0; }
  ...
  for (;;) {
    ...
    if (multiComment) {
      if (c == '\r') {
        ...
        SYNTAX_ERROR("unclosed std::string");
      else if ((c == '*') && (*(curp + 1) == '/')) {
        curp += 2;
        c = *curp;
        if (c == '\r' || c == '\n')
...
static void makeFullPathFromRelative(std::string &path, const char *base_filename) {
...
#define ADD_PARAM_CHECKED(NAME, TYPE, VALUE)                                                                 \
  if (blk.addParam((NAME), (TYPE), (VALUE).c_str(), (VALUE).end(), curLine, fileName) < 0) {                 \
    LOGE("DataBlockParser: invalid value '{}' at line {} of file '{}'", (VALUE).c_str(), curLine, fileName); \
    return false;                                                                                            \
  }
...
inline const char *parse_naive(const char *__restrict value, const char *__restrict eof, int &v) {
  ...
  if (endPtr) {
    ...
    if (endPtr != endPtr2 || v != i2) {
      logerr("Parsed incorrectly, {} != {}, <{}>", v, i2, value);
      return nullptr;
    }
  }
#endif
...
#define PARSING_VALUE_SYNTAX_ERROR(msg)                                                                        \
  {                                                                                                            \
    issue_error_parsing(filename, line, msg, fmt::format("{} (type {}) value {}", name, type, value).c_str()); \
    return -1;                                                                                                 \
  }


int DataBlock::addParam(const char *name, int type, const char *value, const char *eof, int line, const char *filename,
                        int at) {
  toOwned();
  if (at < 0)
    at = paramsCount;
  ...
  switch (type) {
    ...
    case TYPE_IPOINT4:
      ...
      break;
    case TYPE_BOOL: {
      if (dd_stricmp(value, "yes") == 0 || dd_stricmp(value, "on") == 0 || dd_stricmp(value, "true") == 0 ||
          dd_stricmp(value, "1") == 0)
        *(bool *) buf = true;
  ...
  for (;;) {
    ...
    if (*curp == '{') {
      ...
      if (name[0] != '@' || DataBlock::parseOverridesNotApply) {
        ...
        if (!nb) {
          LOGE("cannot find block <{}> (ordinal {}) for override in file {}", blk_name, ord, blk.resolveFilename(true));
        }
        (void) ord;
        ...
        if (nid != -2) {
          LOGE("cannot find block {} (ordinal {}) for deletion in file {}", blk_name, ord, blk.resolveFilename(true));
        }
        (void) ord;
        ...
        if (blk.blockCount() > 0)
          ...
          LOGE("cannot find block for {} in file {}", name.c_str(), blk.resolveFilename(true));
      ...
      if (name[0] != '@' || DataBlock::parseOverridesNotApply) {
        ...
        if (nid >= 0) {
          ...
          if (i >= 0 && i < blk.paramCount()) {
            if (type != blk.getParamType(i) && !DataBlock::allowVarTypeChange)
              LOGE("different types ({} != {}) of param <{}> (ordinal {}) for override in file {}", type,
        ...
        if (nid != -2) {
          LOGE("cannot find param <{}> (ordinal {}) for override in file {}", pname, ord, blk.resolveFilename(true));
        }
        (void) ord;
      } else if (strncmp(name, "@delete:", 8) == 0) {
        ...
        if (nid != -2) {
          LOGE("cannot find param {} (ordinal {}) for deletion in file {}", pname, ord, blk.resolveFilename(true));
        }
        (void) ord;
      } else if (strncmp(name, "@delete-all:", 12) == 0) {
            ...
            if (blk.getParamNameId(i) == nid) {
              if (idx == 0) {
                if (DataBlock::TYPE_STRING != blk.getParamType(i))
                  LOGE("different types ({} != {}) of param <{}> (ordinal {}) for override in file {}",
                       (uint8_t) DataBlock::TYPE_STRING, blk.getParamType(i), pname, ord, blk.resolveFilename(true));
                else
                  blk.setStr(i, value);
                nid = -2;
                break;
      ...
      std::string cachedFileName(fileName);
      ...
      if (!fresolve || !fresolve->resolveIncludeFile(valueStr))
        if (*valueStr.c_str() != '%')
          makeFullPathFromRelative(valueStr, cachedFileName.c_str());

      if (fnotify)
        fnotify->onFileLoaded(valueStr.c_str());

      includeStack.push_back(valueStr);
      fileName = includeStack.back().c_str();

      auto h = file_mgr.getFile(valueStr);
      if (!h) {
        includeStack.pop_back();
        fileName = includeStack.back().c_str();
        blkRef->issue_error_parsing(cachedFileName.c_str(), include_curLine,
                                    fmt::format("can't open include file '{}'", valueStr).c_str(), " ");
        return false;
      }
      ...
      if (len < 0) {
        ...
        fileName = includeStack.back().c_str();
        blkRef->issue_error_parsing(cachedFileName.c_str(), include_curLine,
                                    fmt::format("error loading include file '{}'", valueStr).c_str(), " ");
        return false;
      }
      ...
      *(char *) curp = EOF_CHAR;
      ...
      if (h->df_read(&buffer[pos], len) != len) {
      ...
      if ((/*new binaryformats*/ len > 1 && buffer[pos] >= dblk::BBF_full_binary_in_stream &&
        ...
        LOGE("including binary file '{}', not fastest codepath", valueStr);
        ...
        struct VromfsInPlaceMemLoadCB : public InPlaceMemLoadCB {
          ...
          VromfsInPlaceMemLoadCB(const void *p, int s, File *f) : InPlaceMemLoadCB(p, s), fp(f) {}

          const VROMFs *getTargetVromFs() const override { return fp->getUnderlyingVromfs(); }
        };
        VromfsInPlaceMemLoadCB crd(&buffer[pos], len, h.get());
        inc_blk.loadFromStream(crd, valueStr.c_str(), fnotify, len);
        erase_items(buffer, pos, len + 2);

        DynamicMemGeneralSaveCB cwr(len * 2, 4 << 10);
...
bool DataBlock::load(const char *fname, DataBlock::IFileNotify *fnotify) {
  ...
  FullFileLoadCB crd(fname, true);
  std::string fileName_stor;
  if (!crd.fileHandle && !dd_get_fname_ext(fname)) {
    fileName_stor = fmt::format("{}.blk", fname);
    if (crd.open(fileName_stor, true))
      fname = fileName_stor.c_str();
  }
  if (!crd.fileHandle) {
    if (!shared->blkRobustLoad())
      LOGE("BLK: failed to open file \"{}\" ({})", fname, fileName_stor);
  ...
  const int len = crd.getTargetDataSize() < 0 ? crd.fileHandle->length() : crd.getTargetDataSize();
  if (len < 0) {
    shared->setBlkValid(false);
...
namespace dblk {
  DBNameMap *get_vromfs_shared_name_map(const VROMFs *fs) {
    if (fs)
      return fs->getVromfsSharedNameMap();
    return nullptr;
  }

  ZSTD_DDict_s *get_vromfs_blk_ddict(const VROMFs *fs) {
    if (fs)
      return fs->getVromfsBlkDDict();
    return nullptr;
  }

  bool check_shared_name_map_valid(const VROMFs *fs, const char **out_err_desc) {
    if (fs && fs->getVromfsSharedNameMap())
      return true;
    if (!fs)
...
bool DataBlock::loadFromStream(IGenLoad &crd, const char *fname, DataBlock::IFileNotify *fnotify, unsigned hint_size) {
  ...
  ZSTD_DDict_s *ddict = nullptr;
  {
    ...
    if (label == dblk::BBF_binary_with_shared_nm || label == dblk::BBF_binary_with_shared_nm_z ||
      ...
      const VROMFs *fs = crd.getTargetVromFs();
      ...
      std::span<char> rom_file_data = crd.getTargetRomData();
      if (!rom_file_data.data()) {
        shared->setBlkValid(true);
        issue_error_load_failed(fname, fmt::format("bad ROM data for file: rom=({},{}) crd.tell()={})",
                                                   fmt::ptr(rom_file_data.data()), rom_file_data.size(), crd.tell())
                                         .c_str());
        return false;
      }
...
bool DataBlock::resolveIncludePath(std::string &inout_fname) { return fresolve->resolveIncludeFile(inout_fname); }
