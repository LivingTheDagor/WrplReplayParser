  ...
  return false
    ...
    for (uint32_t i = 0, e = paramCount(); i < e; ++i, ++oldParams) {
        ...
        if (p.type == TYPE_STRING) {
            ...
            char buf[256];
            switch (p.type) {
                case TYPE_BOOL: snprintf(buf, sizeof(buf), *paramData ? "yes" : "no");
                    break;
                case TYPE_INT: snprintf(buf, sizeof(buf), "%d", cdi[0]);
                    break;
                case TYPE_REAL: snprintf(buf, sizeof(buf), "%g", cd[0]);
                    break;
                case TYPE_POINT2: snprintf(buf, sizeof(buf), "%g, %g", cd[0], cd[1]);
                    break;
                case TYPE_POINT3: snprintf(buf, sizeof(buf), "%g, %g, %g", cd[0], cd[1], cd[2]);
                    break;
                case TYPE_POINT4: snprintf(buf, sizeof(buf), "%g, %g, %g, %g", cd[0], cd[1], cd[2], cd[3]);
                    break;
                case TYPE_IPOINT2: snprintf(buf, sizeof(buf), "%d, %d", cdi[0], cdi[1]);
                    break;
                case TYPE_IPOINT3: snprintf(buf, sizeof(buf), "%d, %d, %d", cdi[0], cdi[1], cdi[2]);
                    break;
                case TYPE_IPOINT4: snprintf(buf, sizeof(buf), "%d, %d, %d, %d", cdi[0], cdi[1], cdi[2], cdi[3]);
                    break;
                case TYPE_E3DCOLOR: {
                ...
                case TYPE_MATRIX:
                    snprintf(buf, sizeof(buf), "[[%g, %g, %g] [%g, %g, %g] [%g, %g, %g] [%g, %g, %g]]", cd[0 * 3 + 0],
                             cd[0 * 3 + 1],
                             cd[0 * 3 + 2], cd[1 * 3 + 0], cd[1 * 3 + 1], cd[1 * 3 + 2], cd[2 * 3 + 0], cd[2 * 3 + 1],
                             cd[2 * 3 + 2], cd[3 * 3 + 0],
                             cd[3 * 3 + 1], cd[3 * 3 + 2]);
                    break;
                case TYPE_INT64: snprintf(buf, sizeof(buf), "%lld", (long long int) *cdi64);
                    break;
                default:
    ...
    for (uint32_t i = 0, e = blockCount(); i < e; ++i) {
        ...
        cb.write("{", 1);
        if (print_with_limits &&max_levels
        ==
        1
        ) {
            cb.write(" ... }", 6);
        ...
        if (
            !b.writeText<print_with_limits>(cb, level >= 0 ? level + 1 : level, max_lines,
                                            print_with_limits ? max_levels - 1 : max_levels))
            return false;
        if (level > 0)
            writeIndent(cb, level * 2);
        cb.write("}", 1);
...
bool DataBlock::saveToTextStreamCompact(IGenSave &cwr) const {
    //try {
    writeText < false > (cwr, -1, nullptr, 0);
    //}
    //catch(const IGenSave::SaveException &) { return false; }
    return true;
}

bool DataBlock::saveToTextStream(IGenSave &cwr) const {
    //DAGOR_TRY {
    writeText < false > (cwr, 0, nullptr, 0);
    //}
    //DAGOR_CATCH(const IGenSave::SaveException &) { return false; }
    return true;
}

bool DataBlock::printBlock(std::basic_ostream<char> &cb) const {
    // this->length * 8 is just some value
    DynamicMemGeneralSaveCB cwr(4 << 20, 4 << 20);
...
struct ComplexDataContainer {
    ...
    void alignTo(size_t al) {
        ...
        if ((data.size() & al) != 0) {
            ...
            memset(&(*data.insert(data.end(), add)), 0, add);
        }
    }
    ...
    uint32_t addDataId(const char *val, uint32_t len, hash_t hash) {
        int it = hashToKeyId.findOr(hash, -1,
                                    [&, this](uint32_t id) {
                                        return keys[id].sz == len && memcmp(data.data() + keys[id].ofs, val, len) == 0;
                                    });
        if (it != -1)
            return keys[it].ofs;
        const uint32_t ofs = data.size();
        memcpy(&(*data.insert(data.end(), len)), val, len);
            ...
            for (uint32_t i = 0; i < totalNames; ++i) {
                ...
                if (current != -1) {
                    if (memcmp(name, names.getStringDataUnsafe(current), len) != 0) {
                        ...
                        G_ASSERTF(0, "duplicate string in namemap found <%s> <%s>!", name,
                                  names.getStringDataUnsafe(current));
...
bool dblk::read_names(IGenLoad &cr, DBNameMap &names, uint64_t *names_hash) {
    return read_names_base(cr, names, names_hash);
}
...
void DataBlock::saveToBinStreamWithoutNames(const DataBlockShared &names, IGenSave &cwr) const {
    ...
    if (blocks_count <= MAX_BLK_ON_STACK) {
        iterate_blocks_bfs<decltype(bfs_block_iter)>(*this, bfs_block_iter);
    } else
    ...
    for (auto dbP: bfsOrderBlocks) {
        ...
        *cBlock = DataBlockInfo{
            uint32_t(d.nameIdAndFlags & NAME_ID_MASK), d.paramsCount, d.blocksCount,
            d.blocksCount ? currentBlocks : (uint32_t) 0
        };
...
bool DataBlock::loadFromBinDump(IGenLoad &cr, const DBNameMap *ro) {
    ...
    const uint32_t blocksStartsAt = dag::align_up<DataBlock>(
        complexDataSizeAligned + paramsCnt * sizeof(DataBlock::Param));
    const size_t sharedSize = roDataBlocks * sizeof(DataBlock) + blocksStartsAt + sizeof(DataBlockShared);
    shared = new(malloc(sharedSize)) DataBlockShared;
    ...
    for (uint32_t i = 0, e = shared->roDataBlocks; i < e; ++i, ++blData) {
        DataBlockInfo d;
        read_packed(d, cr);
        new(blData) DataBlock(shared, d.nameId - 1, d.paramsCount, d.blocksCount, d.fBlock, cOfs);
...
void DataBlock::setParamsFrom(const RoDataBlock &blk) {
    ...
    for (uint32_t i = 0, ie = blk.paramCount(); i < ie; ++i) {
        ...
        switch (blk.getParamType(i)) {
            case TYPE_STRING: addStr(name, blk.getStr(i));
                break;
            case TYPE_INT: addInt(name, blk.getInt(i));
                break;
            case TYPE_REAL: addReal(name, blk.getReal(i));
                break;
            case TYPE_POINT2: addPoint2(name, blk.getPoint2(i));
                break;
            case TYPE_POINT3: addPoint3(name, blk.getPoint3(i));
                break;
            case TYPE_POINT4: addPoint4(name, blk.getPoint4(i));
                break;
            case TYPE_IPOINT2: addIPoint2(name, blk.getIPoint2(i));
                break;
            case TYPE_IPOINT3: addIPoint3(name, blk.getIPoint3(i));
                break;
            case TYPE_IPOINT4: addIPoint4(name, blk.getIPoint4(i));
                break;
            case TYPE_BOOL: addBool(name, blk.getBool(i));
                break;
            case TYPE_E3DCOLOR: addE3dcolor(name, blk.getE3dcolor(i));
                break;
            case TYPE_MATRIX: addTm(name, blk.getTm(i));
                break;
            case TYPE_INT64: addInt64(name, blk.getInt64(i));
                break;
            default: G_ASSERT(0);
        }
    }
...
namespace dblk {
    ...
    struct OpenDataBlock : public DataBlock {
        ...
        friend bool load_text(DataBlock &blk, std::span<char> text, dblk::ReadFlags flg, const char *fname,
                              DataBlock::IFileNotify *fnotify);

        friend bool load_from_stream(DataBlock &blk, IGenLoad &crd, dblk::ReadFlags flg, const char *fname,
                                     DataBlock::IFileNotify *fnotify,
                                     unsigned hint_size);

        friend bool save_to_text_file_ex(const DataBlock &blk, const char *filename, bool compact);

        friend bool print_to_text_stream_limited(const DataBlock &blk, IGenSave &cwr, int max_ln, int max_lev,
                                                 int init_lev);
    };
} // namespace dblk
...
static bool dblk::save_to_text_file_ex(const DataBlock &blk, const char *filename, bool compact) {
    EXCEPTION("");
    return false;
    //auto h = ::df_open(filename, DF_WRITE | DF_CREATE);
    //if (!h)
    //  return false;
...
bool dblk::save_to_text_file(const DataBlock &blk, const char *filename) {
    return dblk::save_to_text_file_ex(blk, filename, false);
}
...
bool dblk::load_text(DataBlock &blk, std::span<char> text, dblk::ReadFlags flg, const char *fname,
                     DataBlock::IFileNotify *fnotify) {
    PARSE_FLAGS_PROLOGUE();
    bool res = static_cast<dblk::OpenDataBlock &>(blk).loadText(text.data(), text.size(), fname, fnotify);
    RESTORE_FLAGS_EPILOGUE();
    return res;
}

bool dblk::load_from_stream(DataBlock &blk, IGenLoad &crd, dblk::ReadFlags flg, const char *fname,
                            DataBlock::IFileNotify *fnotify,
                            unsigned hint_size) {
    PARSE_FLAGS_PROLOGUE();
