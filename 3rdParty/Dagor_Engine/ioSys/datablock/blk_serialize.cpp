...
#define CHECK_LINES_LIMIT()  \
      ...
      switch (p.type) {
        ...
        case TYPE_MATRIX:
          snprintf(buf, sizeof(buf), "[[%g, %g, %g] [%g, %g, %g] [%g, %g, %g] [%g, %g, %g]]", cd[0 * 3 + 0],
                   cd[0 * 3 + 1], cd[0 * 3 + 2], cd[1 * 3 + 0], cd[1 * 3 + 1], cd[1 * 3 + 2], cd[2 * 3 + 0],
                   cd[2 * 3 + 1], cd[2 * 3 + 2], cd[3 * 3 + 0], cd[3 * 3 + 1], cd[3 * 3 + 2]);
          break;
  ...
  for (uint32_t i = 0, e = blockCount(); i < e; ++i) {
    ...
    if (!b.writeText<print_with_limits>(cb, level >= 0 ? level + 1 : level, max_lines,
...
struct ComplexDataContainer {
  ...
  uint32_t addDataId(const char *val, uint32_t len, hash_t hash) {
    int it = hashToKeyId.findOr(hash, -1, [&, this](uint32_t id) {
...
void DataBlock::saveToBinStreamWithoutNames(const DataBlockShared &names, IGenSave &cwr) const {
  ...
  for (auto dbP: bfsOrderBlocks) {
    auto &d = *dbP;
    *cBlock = DataBlockInfo{uint32_t(d.nameIdAndFlags & NAME_ID_MASK), d.paramsCount, d.blocksCount,
...
bool DataBlock::loadFromBinDump(IGenLoad &cr, const DBNameMap *ro) {
  ...
  const uint32_t blocksStartsAt =
    dag::align_up<DataBlock>(complexDataSizeAligned + paramsCnt * sizeof(DataBlock::Param));
