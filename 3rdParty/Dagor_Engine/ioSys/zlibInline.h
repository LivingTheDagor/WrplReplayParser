...
public:
    ZLibPacker(int level, bool raw_inflate) {
        strm.zalloc = Z_NULL;
        strm.zfree = Z_NULL;
        int err = raw_inflate
                      ? deflateInit2(&strm, level, Z_DEFLATED, -MAX_WBITS, MAX_MEM_LEVEL >= 8 ? 8 : MAX_MEM_LEVEL,
                                     Z_DEFAULT_STRATEGY)
                      : deflateInit(&strm, level);
        if (err != Z_OK)
            EXCEPTION("zlib error {} in {}", err, "deflateInit");
    }

    ~ZLibPacker() {
        int err = deflateEnd(&strm);
        if (err != Z_OK)
            EXCEPTION("zlib error %d in %s", err, "deflateEnd");
    }
    ...
    int pack(void *&inp, unsigned &insz, void *&outp, unsigned &outsz, int flush) {
        ...
        if (err != Z_OK && err != Z_STREAM_END && err != Z_BUF_ERROR)
            EXCEPTION("zlib error %d in %s", err, "deflate");
        return err;
    }
};
...
public:
    ...
    ZLibGeneralWriter(IGenSave &cb, unsigned bufsz, int level, bool raw_inflate) : ZLibPacker(level, raw_inflate),
        callback(&cb) {
        buf.resize(bufsz);
        uncompressedTotal = 0;
        compressedTotal = 0;
