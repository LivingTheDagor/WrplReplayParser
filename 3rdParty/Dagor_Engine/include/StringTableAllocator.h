...
#ifndef MYEXTENSION_STRINGTABLEALLOCATOR_H
#define MYEXTENSION_STRINGTABLEALLOCATOR_H
...
struct StringTableAllocator {
  ...
  struct StringPage {
    ...
    uint32_t left{};
    uint32_t used{};
    [[nodiscard]] bool empty() const { return bool(data); }
    StringPage() = default;
    StringPage(std::unique_ptr<char[]> &&d, uint32_t l, uint32_t u) : data(std::move(d)), left(l), used(u) {}
    StringPage(StringPage &&a) noexcept : data(std::move(a.data)), left(a.left), used(a.used) { a.reset(); }
    StringPage &operator=(StringPage &&a) noexcept {
      data = std::move(a.data);
      left = std::move(a.left);
      used = std::move(a.used);
      a.reset();
      return *this;
    }
    void reset() {
  ...
  [[nodiscard]] inline uint32_t get_page_ofs(uint32_t addr) const { return addr & ((1 << max_page_shift) - 1); }
  [[nodiscard]] inline uint32_t get_page_id(uint32_t addr) const { return addr >> max_page_shift; }
  ...
  StringPage *addPageUnsafe(uint32_t len, char *data) {
    if (head.data || !pages.empty())
      pages.emplace_back(std::move(head));
    head = StringPage{std::unique_ptr<char[]>(data), len, 0};
    return &head;
  }
  StringPage *addPageUnsafe(uint32_t len) { return addPageUnsafe(len, new char[len]); }
  static uint8_t min8(uint8_t a, uint8_t b) { return a < b ? a : b; }
  static uint8_t max8(uint8_t a, uint8_t b) { return a > b ? a : b; }
  StringPage *addPage(uint32_t len) {
    if (!head.data && pages.empty()) {
  ...
  [[nodiscard]] uint32_t getCurrentHeadOffset() const {
    const auto pagesCount = (uint32_t) pages.size();
  ...
  const char *getDataRawUnsafe(uint32_t ofs,
                               uint32_t &max_len) const // returns pointer to data, if ofs was previously returned by
  ...
  getDataRawUnsafe(uint32_t ofs) const // returns pointer to data, if ofs was previously returned by addDataRaw
  {
    uint32_t pageId, pageOfs;
    get_page_addr(ofs, pageId, pageOfs);
  ...
  [[nodiscard]] size_t totalUsed() const {
  ...
  explicit StringTableAllocator(uint8_t start_page_shift_ = DEFAULT_PAGE_SIZE_SHIFT,
                                uint8_t max_page_shift_ = MAX_PAGE_SIZE_SHIFT) :
...
public:
  ...
  explicit ThreadedStringTableAllocator(uint8_t start_page_shift_ = StringTableAllocator::DEFAULT_PAGE_SIZE_SHIFT,
