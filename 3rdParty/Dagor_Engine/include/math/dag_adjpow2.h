...
inline uint32_t get_log2i(uint32_t v) // branchless version
{
...
inline uint32_t get_log2i_unsafe(uint32_t v) // branchless version, assumes v>0, otherwise UNDEFINED
{
#if HAS_BIT_SCAN_FORWARD
...
inline uint16_t get_log2w(uint16_t v) // branchless version
{
#if HAS_BIT_SCAN_FORWARD
  return get_log2i(v);
#else
...
// 1 for 2, etc
inline uint32_t get_bigger_log2_unsafe(uint32_t v)
{
#if HAS_BIT_SCAN_FORWARD
