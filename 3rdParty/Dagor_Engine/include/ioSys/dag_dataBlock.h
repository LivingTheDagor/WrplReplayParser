...
namespace dblk {
    enum class ReadFlag : uint8_t {
        ...
        ALLOW_SS = 8, //< allow simple std::string during load call (see DataBlock::allowSimpleString)
...
public:
    ...
    //! when true, includes are not resolved but added as 'special' std::string params
    ...
    //! when true, comments are not removed but added as 'special' std::string params
    ...
    struct IIncludeFileResolver {
        virtual bool resolveIncludeFile(std::string &inout_fname) = 0;
    };
    ...
    static bool resolveIncludePath(std::string &inout_fname);

public:
    /// Parameter types enum.
    enum ParamType : uint8_t {
        TYPE_NONE,
        TYPE_STRING, ///< Text std::string.
        TYPE_INT, ///< Integer.
  ...
  /// If you want error checking, use default constructor and load().
    explicit DataBlock(const char *filename);
  ...
  /// @{
    ...
    bool saveToTextStream(IGenSave &cwr) const;

    /// print block to arbitrary stdlib stream (text form). can be used with std::cout
    bool printBlock(std::basic_ostream<char> &out) const;

    /// Save this DataBlock (and its sub-tree) to the arbitrary stream in compact form (text form)
    bool saveToTextStreamCompact(IGenSave &cwr) const;

    /// Save this DataBlock (and its sub-tree) to arbitrary stream (binary form)
    bool saveToStream(IGenSave &cwr) const;
    ...
    inline bool printToTextStreamLimited(IGenSave &cwr, int max_out_line_num = -1, int max_level_depth = -1,
                                         int init_indent = 0) const;
  ...
  /// Use for enumeration.
    ...
    const DataBlock *getBlockByNameEx(std::string_view name, const DataBlock *def_blk) const {
        return getBlockByNameEx(name.data(), def_blk);
    }
    ...
    const DataBlock *getBlockByNameEx(std::string_view name) const { return getBlockByNameEx(name.data()); }
  ...
  /// Returns false on error.
    bool swapBlocks(uint32_t i1, uint32_t i2);
...
#undef TYPE_FUNCTION_3
  ...
  /// Returns false if no parameters were removed.
    ...
    /// Swaps two blocks with specified indices.
  /// Returns false on error.
    bool swapParams(uint32_t i1, uint32_t i2);

    void appendNamemapToSharedNamemap(DBNameMap &shared_nm, const DBNameMap *skip = nullptr) const;

    bool saveBinDumpWithSharedNamemap(IGenSave &cwr, const DBNameMap *shared_nm, bool pack = false,
                                      const ZSTD_CDict_s *dict = nullptr) const;

    bool loadBinDumpWithSharedNamemap(IGenLoad &crd, const DBNameMap *shared_nm, const ZSTD_DDict_s *dict = nullptr);

    /// Save DataBlock tree to binary stream (optionally using namemap stored separately)
    bool saveDumpToBinStream(IGenSave &cwr, const DBNameMap *ro) const;

    /// Load DataBlock tree from binary stream using namemap stored separately
    bool loadFromBinDump(IGenLoad &cr, const DBNameMap *ro);
...
protected:
    ...
    bool loadFromStream(IGenLoad &crd, const char *fname, DataBlock::IFileNotify *fnotify, unsigned hint_size);
    ...
    int addParam(const char *name, int type, const char *value, const char *val_end, int line, const char *filename,
                 int at = -1);
    ...
    uint32_t insertNewString(string_t v, size_t vlen); // returns std::string id
    template<class T, bool rw>
    ...
    __forceinline void insertNewParamRaw(uint32_t pid, uint32_t nameId, uint32_t type, size_t type_size,
                                         const char *data);
...
namespace dblk {
    ...
    bool load(DataBlock &blk, const char *fname, ReadFlags flg = ReadFlags(),
              DataBlock::IFileNotify *fnotify = nullptr);

    inline bool load(DataBlock &blk, std::string_view fname, ReadFlags flg = ReadFlags(),
                     DataBlock::IFileNotify *fnotify = nullptr) {
        return load(blk, fname.data(), flg, fnotify);
    }


    /// Load BLK from text (fname is used only for error reporting)
    bool load_text(DataBlock &blk, std::span<char> text, ReadFlags flg = ReadFlags(), const char *fname = nullptr,
                   DataBlock::IFileNotify *fnotify = nullptr);

    /// Load BLK from stream (fname is used only for error reporting)
    bool load_from_stream(DataBlock &blk, IGenLoad &crd, ReadFlags flg = ReadFlags(), const char *fname = nullptr,
                          DataBlock::IFileNotify *fnotify = nullptr, unsigned hint_size = 128);

    /// Save BLK (and its sub-tree) to the specified file (text form)
    bool save_to_text_file(const DataBlock &blk, const char *filename);

    /// Save BLK (and its sub-tree) to the specified file (text form) in compact form
    bool save_to_text_file_compact(const DataBlock &blk, const char *filename);

    /// Save BLK (and its sub-tree) to the specified file (binary form)
    bool save_to_binary_file(const DataBlock &blk, const char *filename);

    /// Print this DataBlock (and its sub-tree) to the arbitrary text stream with limitations; returns true when whole BLK is written
    bool print_to_text_stream_limited(const DataBlock &blk, IGenSave &cwr, int max_out_line_num = -1,
                                      int max_level_depth = -1,
                                      int init_indent = 0);

    /// Save BLK (and its sub-tree) to the specified file (binary form, packed format)
    bool pack_to_binary_file(const DataBlock &blk, const char *filename, int approx_sz = 16 << 10);

    /// Save BLK (and its sub-tree) to arbitrary stream (binary form, packed format)
    void pack_to_stream(const DataBlock &blk, IGenSave &cwr, int approx_sz = 16 << 10);

    /// Export BLK as JSON to the arbitrary stream (text form)
    bool export_to_json_text_stream(const DataBlock &blk, IGenSave &cwr, bool allow_unquoted = false,
                                    int max_param_per_ln = 4,
                                    int max_block_per_ln = 1);


    /// returns currently set flags of BLK object
    ReadFlags get_flags(const DataBlock &blk);

    /// sets additional flags to BLK object
    void set_flag(DataBlock &blk, ReadFlags flg_to_add);

    /// clear flags of BLK object
    void clr_flag(DataBlock &blk, ReadFlags flg_to_clr);


    static inline const char *resolve_type(uint32_t type) {
        static const char *types[DataBlock::TYPE_COUNT + 1] = {
            "none", "std::string", "int", "real", "point2", "point3", "point4", "ipoint2",
    ...
    static inline bool is_ident_char(char c) {
        return ((c >= 'a' && c <= 'z') || (c >= '0' && c <= '9') || c == '_' || (c >= 'A' && c <= 'Z') || c == '-' || c
                == '.' || c == '~');
    }

    extern const char *SHARED_NAMEMAP_FNAME;

    DBNameMap *create_db_names();

    void destroy_db_names(DBNameMap *nm);

    bool write_names(IGenSave &cwr, const DBNameMap &names, uint64_t *names_hash);

    bool read_names(IGenLoad & cr, DBNameMap & names, uint64_t * names_hash);

    int db_names_count(DBNameMap *);

    //! returns shared name map for specified vromfs (acquires reference)
    DBNameMap *get_vromfs_shared_name_map(const VROMFs *fs);

    //! releases reference to shared name map; returns residual ref count
    int release_vromfs_shared_name_map(DBNameMap *nm);

    //! returns ZSTD decoder dictionary for specified vromfs (acquires reference)
    ZSTD_DDict_s *get_vromfs_blk_ddict(const VROMFs *fs);

    //! releases reference to ZSTD decoder dictionary; returns residual ref count
    int release_vromfs_blk_ddict(ZSTD_DDict_s *ddict);

    //! returns the hash of ZSTD dictionary corresponding to given shared namemap
    std::span<char> get_vromfs_dict_hash(std::span<char> shared_nm_data);

    //! discard unused shared name maps (that were allocated in get_vromfs_shared_name_map() calls); return residual used count
    int discard_unused_shared_name_maps();

    //! discard unused ZSTD decoder dictionaries (that were allocated in get_vromfs_blk_ddict() calls); return residual used count
    int discard_unused_blk_ddict();
    ...
    //ZSTD_CDict_s *create_vromfs_blk_cdict(const VirtualRomFsData *fs, int compr_level);

    //! ZSTD-packs a binary stream as if it contained a datablock dump
    void pack_shared_nm_dump_to_stream(IGenSave &cwr, IGenLoad &crd, int sz, int compr_level,
                                       const ZSTD_CDict_s *cdict);

    //! same but with the default (internally defined) compression level
    void pack_shared_nm_dump_to_stream(IGenSave &cwr, IGenLoad &crd, int sz, const ZSTD_CDict_s *cdict);
    ...
    template<typename Cb>
    static inline void iterate_blocks_by_name_id_list(const DataBlock &db, std::span<int> name_ids, Cb &&cb);

    template<int N, typename Cb>
    static inline void iterate_blocks_by_name_list(const DataBlock &db, const std::array<const char *, N> &const_names,
                                                   Cb &&cb);
...
inline bool DataBlock::printToTextStreamLimited(IGenSave &cwr, int max_out_line_num, int max_level_depth,
                                                int init_indent) const {
...
template<typename Cb>
static inline void dblk::iterate_blocks_by_name_id_list(const DataBlock &blk, std::span<int> name_ids, Cb &&cb) {
...
template<int N, typename Cb>
static inline void dblk::iterate_blocks_by_name_list(const DataBlock &blk,
                                                     const std::array<const char *, N> &const_names, Cb &&cb) {
    std::array < int, N > nameIds;
    int cnt = 0;
    for (int i = 0; i < N; i++) {
