...
struct OAHashNameMap {
    ...
    std::vector<uint64_t> strings; // Note: high 32 bits - length of string
    ...
    uint32_t addString(const char *name, size_t name_len) {
        const uint32_t len = (uint32_t) name_len + 1;
        // term zero, or one symbol after len. todo: fixme, so we don't memcpy from it
    ...
    const char *getStringDataUnsafe(uint32_t id, uint32_t &max_len) const {
        return allocator.getDataRawUnsafe(strings[id], max_len);
    }
    ...
    inline bool string_equal(const char *name, size_t name_len, const uint32_t id) const {
        ...
        return (max_len >= name_len ? memcmp(str, name, name_len) : strncmp(str, name, name_len)) == 0;
        // we actually probably don't ever
        // collide
    }
    ...
    int getNameId(const char *name, size_t name_len) const {
        return getNameId(name, name_len, string_hash(name, name_len));
    }
    ...
    void erase(uint32_t id) {
        ...
        if (DAGOR_LIKELY(noCollisions()))
            ...
            hashToStringId.erase(hash, [this, name, name_len](uint32_t id) {
                return string_equal(name, name_len, id);
            });
...
struct FixedCapacityOAHashNameMap {
    ...
    int getNameId(const char *name, size_t name_len, const hash_t hash) const {
        return hashToStringId.findOr(hash, -1, [this, name, name_len](uint32_t id) {
            return string_equal(name, name_len, id);
        });
    }
...
static void gather_ids_in_lexical_order(T &out_sorted_stor, const OAHashNameMap<CI, DefaultOAHasher<CI> > &nm) {
    ...
    for (int i = 0; i < out_sorted_stor.size(); i++)
        out_sorted_stor[i] = i;
    std::sort(out_sorted_stor.begin(), out_sorted_stor.end(), [&nm](const int a, const int b) {
...
template<typename CB, typename TAB, typename NM>
static inline IterateStatus iterate_names_in_order_breakable(const NM &nm, const TAB &ids,
