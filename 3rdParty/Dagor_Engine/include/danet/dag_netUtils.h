...
namespace netutils
{
  void writeCompressedBuffer(BitStream &msg, std::vector<uint8_t> buf);
  ...
  inline T packun(float s, float _max, uint32_t n, const char *fu, const char *f, int l, const char *msg = "")
  {
    ...
    if (s > _max)
      EXCEPTION("%s(%d) : pack overlow in %s max = %f, val = %f, msg = %s", f, l, fu, _max, s, msg);
    else if (s < 0.f)
      EXCEPTION("%s(%d) : pack error (val is lesser then 0.f) in %s val = %f, msg = %s", f, l, fu, s, msg);
  ...
  inline T packu_mm(float s, float _min, float _max, const char *fu, const char *f, int l, const char *msg = "")
  {
    ...
    if (s > _max)
      ...
      EXCEPTION("%s(%d) : pack error (val is lesser then %f) in %s val = %f, msg = %s", f, l, _min, fu, s, msg);
