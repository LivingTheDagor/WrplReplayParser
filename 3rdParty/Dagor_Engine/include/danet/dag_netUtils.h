...
namespace netutils
{
//void writeCompressedBuffer(BitStream &msg, dag::ConstSpan<uint8_t> buf);
//bool readCompressedBuffer(const BitStream &msg, std::vector<uint8_t> &buf);

//void writeCompressedBlk(BitStream *message, const DataBlock &blk);
//bool readCompressedBlk(const BitStream *message, DataBlock &blk);
...
inline constexpr int8_t get_invalid_s(int8_t) { return INT8_MIN; }
inline constexpr int16_t get_invalid_s(int16_t) { return INT16_MIN; }
...
inline T packun(float s, float _max, uint32_t n, const char *fu, const char *f, int l, const char *msg = "")
{
  ...
  if (s > _max)
    LOGE("{}({}) : pack overflow in {} max = {}, val = {}, msg = {}", f, l, fu, _max, s, msg);
  else if (s < 0.f)
    LOGE("{}({}) : pack error (val is lesser then 0.f) in {} val = {}, msg = {}", f, l, fu, s, msg);
...
template <typename T>
inline float unpacks_dbg(T s, float _max, const char *fn, const char *file, int l, const char *msg = "")
{
  G_LOGERR_AND_DO(s != get_invalid_s(T()), return -_max, "{}({}) : failed to unpack val={} in {}() msg:'{}'", file, l, s, fn, msg);
  G_UNUSED(fn);
  G_UNUSED(file);
  G_UNUSED(l);
  G_UNUSED(msg);
  return UNPACKS(s, _max);
}
...
inline T packu_mm(float s, float _min, float _max, const char *fu, const char *f, int l, const char *msg = "")
{
  ...
  if (s > _max)
    ...
    LOGE("{}({}) : pack error (val is lesser then {}) in {} val = {}, msg = {}", f, l, _min, fu, s, msg);
...
std::array<int16_t, 3> pack_euler_16(const Point3 &euler);
void unpack_euler_16(std::array<int16_t, 3> packed, Point3 &res);
...
#define UNPACK_S16(s, _max) netutils::unpacks_dbg<int16_t>(s, _max, __FUNCTION__, __FILE__, __LINE__)
#define UNPACK_S8(s, _max)  netutils::unpacks_dbg<int8_t>(s, _max, __FUNCTION__, __FILE__, __LINE__)

#define UNPACK_U16(s, _max) netutils::unpacks_dbg<uint16_t>(s, _max, __FUNCTION__, __FILE__, __LINE__)
#define UNPACK_U8(s, _max)  netutils::unpacks_dbg<uint8_t>(s, _max, __FUNCTION__, __FILE__, __LINE__)
...
#define UNPACK_S16_MSG(s, _max, msg) netutils::unpacks_dbg<int16_t>(s, _max, __FUNCTION__, __FILE__, __LINE__, msg)
#define UNPACK_S8_MSG(s, _max, msg)  netutils::unpacks_dbg<int8_t>(s, _max, __FUNCTION__, __FILE__, __LINE__, msg)
...
void write_float(BitStream &bs, float val, const FloatSerializeProps &props);
bool read_float(const BitStream &bs, float &val, const FloatSerializeProps &props);

void write_dir(BitStream &bs, const Point3 &dir);
bool read_dir(const BitStream &bs, Point3 &dir);

void write_idx(BitStream &bs, int idx);
bool read_idx(const BitStream &bs, int &idx);

void write_bitarray(BitStream &bs, const Bitarray &ba);
bool read_bitarray(const BitStream &bs, Bitarray &ba);

void write_euler(BitStream &bs, const Point3 &euler);
bool read_euler(const BitStream &bs, Point3 &euler);

void write_vector(BitStream &bs, const Point3 &pos, float max);
bool read_vector(const BitStream &bs, Point3 &pos, float max);
} // namespace netutils