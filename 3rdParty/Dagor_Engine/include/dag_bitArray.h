...
public:
  ...
  /// Move constructor
  Bitarray(Bitarray &&a) : total(a.total), used(a.used), ptr(a.ptr) { a.ptr = NULL; }
  ...
  */
  ...
  bool reserve(int n)
  {
    if (n <= 0 || n + used < (total << BitarraybitsSHIFT))
      return true;
    size_t s = get_size_bytes(n + used);
    Bitarraybits *p = (Bitarraybits *)realloc(ptr, get_size_bytes(n + used));
    if (p)
    {
      total = int(s / sizeof(Bitarraybits));
      ptr = p;
  ...
  bool resize(int c)
  {
    ...
    size_t s = get_size_bytes(c);
    Bitarraybits *p = (Bitarraybits *)realloc(ptr, get_size_bytes(c));
    if (p)
    {
      int ou = -1;
      if (used < c)
  ...
  void shrink(void)
  {
    size_t s = get_size_bytes(used);
    ptr = (Bitarraybits *)realloc(ptr, get_size_bytes(used));
    total = int(s / sizeof(Bitarraybits));
  }
