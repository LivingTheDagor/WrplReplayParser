...
public:
  ...
  iterator insert(const_iterator at, value_type &&v)
  {
    if (EASTL_LIKELY(data() > &v || data() + capacity() <= &v))
      return new (insert_uninitialized(at)) value_type((value_type &&) v);
    value_type tmp((value_type &&) v);
    return new (insert_uninitialized(at)) value_type((value_type &&) tmp);
  }
  iterator insert(const_iterator at, const value_type &v) { return new (insert_uninitialized(at)) value_type(v); }
  ...
  {
    return new (insert_uninitialized(at)) value_type(eastl::forward<Args>(args)...);
  }

  value_type &push_back() { return *base_type::template defCtor<zero_init_scalars>(push_back_uninitialized()); }
  value_type &push_back(const value_type &v) { return *(new (push_back_uninitialized()) value_type(v)); }
  value_type &push_back(value_type &&v) { return *(new (push_back_uninitialized()) value_type((value_type &&) v)); }
  ...
  {
    return *new (push_back_uninitialized()) value_type(eastl::forward<Args>(args)...);
  }
...
protected:
  ...
  void allocateAndCopy(size_type count, const value_type *p) // no Free! internal function to avoid copy-paste!
  {
    ...
    for (auto end = dst + count; dst != end; ++dst, ++p)
      new (dst) value_type(*p); // zero initializing scalars
  }
...
inline void RelocatableFixedVector<T, N, O, A, C, Z>::assign(const value_type *s, const value_type *e)
{
  ...
  if (newCount > size())
    for (auto i = cData + oldCount, ie = cData + newCount; i != ie; ++i, ++s)
      new (i) value_type(*s); // unitialized copy
  else
...
inline void RelocatableFixedVector<T, N, O, A, C, Z>::assign(size_type newCount, const value_type &v)
{
  ...
  if (newCount > oldCount)
    for (auto i = cData + oldCount, ie = cData + newCount; i != ie; ++i)
      new (i) value_type(v); // unitialized copy
  else
    for (auto i = cData + newCount, ie = cData + oldCount; i != ie; ++i)
      i->~value_type();
  base_type::used() = newCount;
