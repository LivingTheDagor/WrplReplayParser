    ...
    if (CMAKE_SYSTEM_NAME STREQUAL "Windows")
        ...
        elseif (CMAKE_BUILD_TYPE STREQUAL "Debug")
            if (CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
                ...
                set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} /debug" PARENT_SCOPE)
            elseif (CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
                set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -static -O0" PARENT_SCOPE)
            elseif (CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
    ...
    elseif (CMAKE_SYSTEM_NAME STREQUAL "Linux")
        ...
        elseif (CMAKE_BUILD_TYPE STREQUAL "Debug")
            ...
            elseif (CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
                ...
                # set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -fPIC -fsanitize=undefined  -g -O0" PARENT_SCOPE)
            endif ()
            # elseif (CMAKE_BUILD_TYPE STREQUAL "RelWithDebInfo")
            #     if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
            #         set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -O3 -fPIC -fsanitize=address -fno-omit-frame-pointer -g" PARENT_SCOPE)
            #     endif ()
        endif ()
...
# a method I aied to dynamically enable mimalloc flags based on current sanitizer
function(mimalloc_enable_sanitizer_options)
    set(_enable_asan FALSE)
    set(_enable_tsan FALSE)
    set(_enable_ubsan FALSE)

    foreach(_lang C CXX)
        if(CMAKE_${_lang}_FLAGS MATCHES "-fsanitize=address")
            set(_enable_asan TRUE)
        endif()
        if(CMAKE_${_lang}_FLAGS MATCHES "-fsanitize=thread")
            set(_enable_tsan TRUE)
        endif()
        if(CMAKE_${_lang}_FLAGS MATCHES "-fsanitize=undefined")
            set(_enable_ubsan TRUE)
        endif()
    endforeach()

    foreach(_cfg DEBUG RELWITHDEBINFO MINSIZEREL RELEASE)
        string(TOUPPER "${_cfg}" _cfg_upper)
        foreach(_lang C CXX)
            if(DEFINED CMAKE_${_lang}_FLAGS_${_cfg_upper} AND
                    CMAKE_${_lang}_FLAGS_${_cfg_upper} MATCHES "-fsanitize=address")
                set(_enable_asan TRUE)
            endif()
            if(DEFINED CMAKE_${_lang}_FLAGS_${_cfg_upper} AND
                    CMAKE_${_lang}_FLAGS_${_cfg_upper} MATCHES "-fsanitize=thread")
                set(_enable_tsan TRUE)
            endif()
            if(DEFINED CMAKE_${_lang}_FLAGS_${_cfg_upper} AND
                    CMAKE_${_lang}_FLAGS_${_cfg_upper} MATCHES "-fsanitize=undefined")
                set(_enable_ubsan TRUE)
            endif()
        endforeach()
    endforeach()
    if(_enable_asan)
        set(MI_TRACK_ASAN ON CACHE BOOL "Compile with address sanitizer support" FORCE)
    endif()

    if(_enable_tsan)
        set(MI_DEBUG_TSAN ON CACHE BOOL "Build with thread sanitizer" FORCE)
    endif()

    if(_enable_ubsan)
        set(MI_DEBUG_UBSAN ON CACHE BOOL "Build with undefined-behavior sanitizer" FORCE)
        set(MI_USE_CXX ON CACHE BOOL "Use C++ compiler for mimalloc UBSAN build" FORCE)
    endif()
endfunction()