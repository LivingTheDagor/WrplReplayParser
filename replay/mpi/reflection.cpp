...
namespace danet {
  ...
  int ReflectableObject::serialize(BitStream &bs, ParserState *state, uint16_t flags_to_have, bool fth_all,
                                   uint16_t flags_to_ignore, bool fti_all, bool do_reset_changed_flag) {
    checkWatermark();
    ...
    if (numSerializedVars) {
      ...
      for (int i = 0; i < numSerializedVars; ++i) {
        ...
        G_ASSERTF(pos_after_write > pos_before_write,
                  "{} in object '{}' var coder {} for var '{}' ({}) did not writed any data", __FUNCTION__,
                  getClassName(), fmt::ptr((void *) v->coder), v->getVarName(), fmt::ptr(v));
        ...
        if ((option_flags & DUMP_REFLECTION) || (v->flags & RVF_DEBUG) || (reflectionFlags & DEBUG_REFLECTION))
          debug("%s serialize var '%s'(0x%p == 0x%x) %d bytes in obj '%s'(0x%p)", __FUNCTION__, v->getVarName(), v,
                v->getValue<int>(), BITS_TO_BYTES(v->numBits), getClassName(), this);
#endif
      ...
      if (option_flags & DUMP_REFLECTION)
        debug("%s serialized object 0x%p '%s' %u vars %u(+2) bytes", __FUNCTION__, this, getClassName(),
              numSerializedVars, total_data_size);
#endif
    }
  ...
  int serializeChangedReflectables(BitStream &bs, ParserState *state, uint16_t flags_to_have, bool fth_all,
                                   uint16_t flags_to_ignore, bool fti_all, bool do_reset_changed_flag) {
    ...
    for (ReflectableObject *r = changed_reflectables.head; r;) {
      ...
      if ((r->isRelfectionFlagSet(ReflectableObject::EXCLUDED)) == 0 && r->isNeedSerialize()) {
        ...
        if (r->serialize(bs, state, flags_to_have, fth_all, flags_to_ignore, fti_all, do_reset_changed_flag)) {
          ...
          (void) data_written;
          G_ASSERTF(data_written <= 65535, "{} {} type='{}' writed more than 65535 byte data ({})", __FUNCTION__,
                    fmt::ptr(r), r->getClassName(), data_written);
  ...
  int serializeAllReflectables(BitStream &bs, ParserState *state, uint16_t flags_to_have, bool fth_all,
                               uint16_t flags_to_ignore, bool fti_all, bool do_reset_changed_flag) {
    ...
    for (uint16_t j = 0; j < numVars; ++j) {
      ...
      if (bs.GetReadOffset() - ppp != idFieldSerializer.getFieldSize(j)) {
        if (out != 2) {
          ...
          LOGE("(REFLECTION) Invalid parsed size for var '{}', needed {} but parsed {}", v->getVarName(),
               idFieldSerializer.getFieldSize(j), bs.GetReadOffset() - ppp);
        }
  ...
  int deserializeReflectables(BitStream &bs, mpi::object_dispatcher resolver, ParserState *state) {
    ...
    for (int i = 0; i < numReflectables; ++i) {
      ...
      G_ASSERTF(BITS_TO_BYTES(bs.GetNumberOfUnreadBits()) >= data_written,
                "{} unexpected stream end - unreaded {} bytes, data_written={} bytes", __FUNCTION__,
                BITS_TO_BYTES(bs.GetNumberOfUnreadBits()), data_written);
