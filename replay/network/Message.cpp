...
namespace net {
  ...
  uint32_t MessageClass::initImpl(bool server, bool dbg_output_table) {
    ...
    G_ASSERT(
      !server ||
      syncedServerMessageHashes.empty()); // when initializing for server, there must not be any synced message hashes
    if (!server)
      for (uint32_t i = 0; i < syncedServerMessageHashes.size(); i++)
        // their incoming - our outgoing
        (i < syncedClientToServerMsgCount ? outgoingMessagesMap : incomingMessagesMap)
          .emplace(syncedServerMessageHashes[i], nullptr);
    ...
    for (MessageClass *cls = classLinkList; cls; cls = cls->next) {
      ...
      if (EASTL_UNLIKELY(!syncedServerMessageHashes.empty() && !server &&
                         messageMap.find(cls->classHash) == messageMap.end())) {
    ...
    incomingMessagesList.resize(uint32_t(incomingMessagesMap.size()));
    {
      ...
      stlsort::sort_branchless(
        skippedMsgClasses.begin(), skippedMsgClasses.end(),
  ...
  void MessageClass::startWaitingForMessageIdsSync() {
    G_ASSERT(msgSysSyncState == MessageSysSyncState::NOT_SYNCED ||
  ...
  void MessageClass::writeMessageIdsSync(BitStream &bs, uint64_t client_hash) {
