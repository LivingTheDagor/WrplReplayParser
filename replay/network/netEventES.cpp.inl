...
/*inline void entity_events(ecs::EntityManager &mgr, const ecs::Event &event, ecs::EntityId eid) {
  eastl::unique_ptr<net::IMessage> msg(net::event::create_message_by_event(event, net::Er::Unicast));
  if (msg)
    send_net_msg(mgr, eid, eastl::move(*msg));
}

inline void broadcast_events(ecs::EntityManager &mgr, const ecs::Event &event) {
  eastl::unique_ptr<net::IMessage> msg(net::event::create_message_by_event(event, net::Er::Broadcast));
  if (msg)
    send_net_msg(mgr, net::get_msg_sink(mgr.owned_by), eastl::move(*msg));
}

ECS_ON_EVENT(EventNetEventDummy) // we actually won't receive this event
inline void net_unicast_events_es_event_handler(const ecs::Event &event, ecs::EntityManager &manager,
                                                ecs::EntityId eid) {
...
static void net_unicast_events_es_event_handler_all_events(ecs::EntityManager &mgr, const ecs::Event &__restrict,
                                                           const ecs::QueryView &__restrict);
static void net_broadcast_events_es_event_handler_all_events(ecs::EntityManager &mgr, const ecs::Event &__restrict,
                                                             const ecs::QueryView &__restrict);
...
namespace net {
  ...
  namespace event {
    ...
    static void claim_for(ecs::EntityManager *mgr, dag::ConstSpan<MessageRouting> tx_routings) {
      LOGI("[net::event::claim_for] mgr={} tx_routings.size={}", fmt::ptr(mgr), (uint32_t) tx_routings.size());
      ...
      // bool didRebuild = false;
      {
        ...
        /*for_each_net_tx_handler(
          [mgr, tx_routings, &unicastCount, &broadcastCount](ecs::EntitySystemDesc *esd, bool unicast) {
            ecs::EventSet evs =
              build_event_set_for_tx_routing(tx_routings, unicast ? net::Er::Unicast : net::Er::Broadcast);
            esd->setEvSet(evs.empty() ? ecs::EventSetBuilder<EventNetEventDummy>::build() : eastl::move(evs));
            // esd->setOwner(mgr);
            (unicast ? unicastCount : broadcastCount)++;
          });*/
        LOGI("[net::event::claim_for] mgr={} claimed unicast={} broadcast={}", fmt::ptr(mgr), unicastCount,
             broadcastCount);
        // if (mgr)
        //   didRebuild = mgr->();
      }
    ...
    /*
        void shutdown() {
          ...
          LOGI("[net::event::shutdown] tid=%lld ENTER", dbgTid);
          ecs::EsListLock::ScopedLock lock;
          int releasedCount = 0;
          ...
          rx_msg_bitmap.clear();
          rx_msg_index.clear();
          LOGI("[net::event::shutdown] tid=%lld DONE releasedHandlers=%d", dbgTid, releasedCount);
        }
        ...
        void release_claim(ecs::EntityManager *mgr) {
          ...
          LOGI("[net::event::release_claim] tid=%lld mgr=%p released=%d", dbgTid, mgr, releasedCount);
        }*/
    ...
    IMessage *create_message_by_event(const ecs::Event &evt, net::Er er, IMemAlloc *alloc) {
      ...
      if (it == tx_events_index.end())
        return nullptr;
      G_ASSERTF(it->second->er == er, "Message %s/%x was registered as broadcasted but sent as unicast (or vice versa)",
                evt.getName(), evt.getType());
