...
namespace net {
  ...
  using HandlerMap = eastl::unordered_map<uint32_t, msg_handler_t>;
  ...
  void register_net_msg_handler(const MessageClass &klass, msg_handler_t handler) {
    ...
    G_ASSERTF(ins.second || (ins.first->second == handler), "Duplicate net msg handler for message class %s/%x",
              klass.debugClassName, klass.classHash);
