...
#define SEQ_HISTORY_DEPTH 1024
...
#if DAGOR_DBGLEVEL > 0
    bool alreadyExist = eastl::find_if(pendingAcks.begin(), pendingAcks.end(), [this](const PendingAckRec &qr) {
                          return qr.sequence == localSequence;
                        }) != pendingAcks.end();
  ...
  void ReliabilitySystem::packetReceived(sequence_t seq) { receivedQueue.insert(seq); }
  ...
  bool Connection::readConstructionPacket(const BitStream &bs, float compression_ratio) {
    ...
    for (int i = 0, n = written + 1; i < n; ++i) {
      ...
      if (DAGOR_LIKELY(mgr->getEntityTemplateId(resolvedEid) == ecs::INVALID_TEMPLATE_INDEX)) {
        ...
        CONN_LOGE("Attempt to create already existing network entity {}<{}>", serverEid,
                  mgr->getEntityTemplateName(resolvedEid));
  ...
  const char *Connection::deserializeTemplate(const BitStream &bs, ecs::template_t &templateId,
                                              bool &tpl_deserialized) {
    // it is too expensive to always serialize template name, so we just rely on construction being reliable ordered,
    // and write name once.
  ...
  bool Connection::syncReadComponent(ecs::component_index_t serverCidx, const BitStream &bs, ecs::template_t templateId,
    ...
    if (!(bs.Read(name) && bs.Read(type)))
      return false;
    ecs::component_index_t clientCidx =
      ecs::g_ecs_data->getDataComponents()->getIndex(name); // try to immediately resolve component
    if (clientCidx == ecs::INVALID_COMPONENT_INDEX) // component is missing on client
    {
      ...
      // G_UNUSED(loglev);
      EXCEPTION("component scidx={}, name={:#x} type={:#x}({}) is missing in template <{}> on client", serverCidx, name,
                type, ecs::g_ecs_data->getComponentTypes()->getName(typeIdx), serverTemplates[templateId].c_str());
      //}
  ...
  bool Connection::deserializeComponentConstruction(ecs::template_t server_template, const BitStream &bs,
    ...
    for (uint16_t comp = 0, i = 0; i < compCount; ++i) {
      ...
      // cmp->getName().data() << "("<<
      // ecs::g_ecs_data->getComponentTypes()->getComponentData(cmp->componentIndex)->name.data() << ")\n";
      // std::cout.flush();
      if (ecs::MaybeComponent mbcomp =
            deserialize_init_component_typeless(componentTypeName, cidx, deserializer, mgr)) {
        if (mbcomp.has_value())
  ...
  bool Connection::writeConstructionPacket(BitStream &bs, BitStream &tmpbs, std::vector<ecs::EntityId> &toWrite) {
...

    {
      ...
      uint32_t blockSizeBytes =
        write_block(bs, tmpbs, [this, eid](BitStream &bs2) { serializeConstruction(eid, bs2); });
  ...
  void Connection::serializeConstruction(ecs::EntityId eid, BitStream &bs, bool canSkipInitial) {
    ...
    else // not yet synced/known to client
    {
      ...
      // todo: actually, we'd better serialize template, if it is different on server and client. Would require some
      // state to check.
    ...
    // const ecs::component_index_t *ignoredComponents = get_template_ignored_initial_components(templateId),
    // *ignoredComponentsE = nullptr; if (ignoredComponents)
    //{
    ...
    iterateReplicatable([&, eid](ecs::ComponentRef comp, uint16_t cid) {
      ...
      if (false) {
        ...
        const uint16_t ofs =
          (writtenComponents++ == 0) ? componentsInTemplate : uint16_t(componentsInTemplate - prevComponent - 1);
        ...
        // LOG("serializing %s at %i\n", ecs::g_entity_mgr->getDataComponents()->getName(comp.getComponentId()).data(),
        // bs.GetWriteOffset());
    ...
    // Remove this check when we sure enough that it's not happens and use bitmap for storing info about count < 256
    // instead
    if (DAGOR_UNLIKELY(componentsInTemplate != serverTemplateComponentsCount[serverWrittenIdx]))
      CONN_LOGE("Inconsistent replication components count %d (cur) != %d (initial) in template %d<%s>",
                componentsInTemplate, serverTemplateComponentsCount[serverWrittenIdx], templateId,
  ...
  bool Connection::readReplicationPacket(const BitStream &bs) {
    ...
    do {
      ...
      } else // attrs update for missing local object (assume that creation packet is not received yet, and save it for
             // future apply)
      {
  ...
  bool Connection::deserializeComponentReplication(ecs::EntityId eid, const BitStream &bs) {
    ...
    CONN_LOGE("{} {}<{:#x}> of type {}<{:#x}> for entity {:#x}<{}>", warn_type, datacomp->getName(), datacomp->hash,
              comp->name, comp->hash, eid.get_handle(), this->mgr->getEntityTemplateName(eid));
  ...
  bool Connection::serializeComponentReplication(ecs::EntityId eid, const ecs::ComponentRef &comp, BitStream &bs) {
    if (!componentsSynced.test(comp.getComponentId(), false)) // FIXME: race on components replication & re-creation
    {
      ...
      CONN_LOGE("Attempt to serialize not-yet synced component <{}> of type <{}>",
                ...
                ecs::g_ecs_data->getComponentTypes()->getName(comp.getTypeId()), eid.get_handle(),
                mgr->getEntityTemplateName(eid));
      return false;
    }
