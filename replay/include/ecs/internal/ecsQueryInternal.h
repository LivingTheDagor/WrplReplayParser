...
namespace ecs {
  ...
  public:
    ...
    Range getRQ() const { return Range{uint16_t(data.rwCnt + data.roCnt), data.rqCnt}; }
    Range getRO() const { return Range{data.rwCnt, data.roCnt}; }
    Range getRW() const { return Range{0, data.rwCnt}; }
    uint16_t getNoStart() const { return uint16_t(data.rwCnt + data.roCnt + data.rqCnt); }
    uint8_t getNoCnt() const { return uint8_t(components.size() - getNoStart()); }
    uint8_t getRqCnt() const { return data.rqCnt; }
    uint8_t getRwCnt() const { return data.rwCnt; }
    uint8_t getRoCnt() const { return data.roCnt; }
