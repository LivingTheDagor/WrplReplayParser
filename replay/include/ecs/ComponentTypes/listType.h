...
namespace ecs {
  ...
  public:
    ...
    T &push_back(Component &&v) {
      G_ASSERTF(v.is<T>(), "Wrong type passed to ecs::List");

      changeGen();
      base_type::emplace_back(v.getNullable<T>());
      return base_type::back();
    }
    ...
    iterator insert(const_iterator position, Component &&v) {
      G_ASSERTF(v.is<T>(), "Wrong type passed to ecs::List");

      changeGen();
      return base_type::insert(position, eastl::move(v.getNullable<T>()));
    }
    ...
    auto getEntityComponentRef(ecs::EntityManager *mgr) const {
      ...
      return EntityComponentRef((void *) (this), ecs::ComponentTypeInfo<ecs::List<T>>::type, ti,
                                INVALID_COMPONENT_INDEX);
    }

    auto getEntityComponentRef(size_t i) const {
      return EntityComponentRef((void *) (data() + i), ecs::ComponentTypeInfo<T>::type, getItemTypeId(),
                                INVALID_COMPONENT_INDEX);
    }
