...
namespace ecs {
  ...
  // I added like 8 files to make this work lmao, with it supposedly being sorted
  // if your confused about what this does, think of it as a weird std::array<event_type_t, 7>
  ...
  enum EventCastType {
    EVCAST_UNKNOWN = 0x00,
    EVCAST_UNICAST = 0x01, // for a single entity
    EVCAST_BROADCAST = 0x02, // for every valid entity
    EVCAST_BOTH = 0x03,
  };
  ...
  enum EventFlags : event_flags_t {
    ...
    EVFLG_SCHEMELESS = 0x10, // actual class is ecs::SchemelessEvent. This is temporarily, as we plan to get rid of all
                             // schemeless events, once we make inspection code
...
#define ECS_INSIDE_EVENT_DECL(Klass, Flags)                                                       \
  ...
  static void destroy(Event &e) {                                                                 \
    ...
    if (!(Klass::staticFlags() & ::ecs::EVFLG_DESTROY)) /*to reduce generated code size*/         \
      return;                                                                                     \
    G_ASSERT(e.getType() == staticType());                                                        \
    static_cast<Klass &>(e).~Klass();                                                             \
  }                                                                                               \
  static void move_out(void *__restrict allocateAt, Event &&from) {                               \
    if (!(Klass::staticFlags() & ::ecs::EVFLG_DESTROY)) /*to reduce generated code size*/         \
      return;                                                                                     \
    G_ASSERT(from.getType() == staticType());                                                     \
    new (allocateAt) Klass(eastl::move(static_cast<Klass &&>(from)));                             \
  }
...
// this is the only way to declare an event,
...
#define ECS_BROADCAST_PROFILE_EVENT_TYPE(Klass, ...) \
  ...
  protected:
    ...
    event_type_t eventType = 0; // hashed name
  ...
  public:
    EventInfoLinkedListInst() :
      inst(T::staticName(), T::staticType(), sizeof(T), T::staticFlags(),
           (T::staticFlags() & EVFLG_DESTROY) ? &T::destroy : nullptr,
           (T::staticFlags() & EVFLG_DESTROY) ? &T::move_out : nullptr) {
      G_STATIC_ASSERT(sizeof(T) < std::numeric_limits<event_size_t>::max());
    }
...
#define ECS_REGISTER_EVENT_NS(ns, event_type, ...) \
  static ecs::EventInfoLinkedListInst<ns ::event_type> ecs_event_type_##event_type;
