...
namespace net {
  ...
  struct EventRegRecord {
    ...
    EventRegRecord(ecs::event_type_t etype, const MessageClass &mcls, Er er_, event2msg_t evt2msg_,
                   msg2event_t msg2evt_) :
      eventType(etype), er(er_), evt2msg(evt2msg_), msg2evt(msg2evt_), msgClass(mcls) {
  ...
  namespace event {
    ...
    bool try_receive(const net::IMessage &msg, ecs::EntityManager &mgr,
                     ecs::EntityId toeid); // return false if message wasn't event one
    IMessage *create_message_by_event(const ecs::Event &evt, net::Er er, IMemAlloc *alloc = &G_ALLOC);
  }; // namespace event
