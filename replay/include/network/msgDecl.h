...
{
  inline void write_type(BitStream &bs, ecs::EntityId eid) { net::write_eid(bs, eid); }
  inline bool read_type(const BitStream &bs, ecs::EntityId &eid) { return net::read_eid(bs, eid); }
} // namespace danet
...
namespace net {
  ...
  namespace detail {
    template<typename T>
    inline void write_value(BitStream &bs, const T &val) {
      bs.Write(val);
    }
    template<typename T>
    inline bool read_value(const BitStream &bs, T &val, const Connection &) {
      return bs.Read(val);
    }
    template<>
    ...
    // inline void write_value(BitStream &, ConnectionId) {} // Nothing is written
    // bool read_value(const BitStream &bs, ConnectionId &cid, const IConnection &conn);

    template<typename T, int I>
    struct Serialize {
      void operator()(const T &t, BitStream &bs) {
    ...
    struct Serialize<T, 0> {
      void operator()(const T &, BitStream &) {}
    };

    template<typename T, int I>
    struct DeSerialize {
      bool operator()(T &t, const BitStream &bs, const Connection &conn) {
        return DeSerialize<T, I - 1>()(t, bs, conn) && read_value(bs, eastl::get<I - 1>(t), conn);
      }
    };
    template<typename T>
    struct DeSerialize<T, 0> {
      bool operator()(T &, const BitStream &, const Connection &) { return true; }
    };
  }; // namespace detail
...
#define ECS_NET_DECL_MSG_EX(Klass, BaseKlass, TU)                                                                \
  struct Klass final : public BaseKlass, TU {                                                                    \
    ...
    Klass(Klass &&) = default;                                                                                   \
    virtual void pack(BitStream &bs) const override {                                                            \
      net::detail::Serialize<Tuple, eastl::tuple_size<Tuple>::value>()(static_cast<const Tuple &>(*this), bs);   \
    }                                                                                                            \
    virtual bool unpack(const BitStream &bs, const net::Connection &conn) override {                             \
      return net::detail::DeSerialize<Tuple, eastl::tuple_size<Tuple>::value>()(static_cast<Tuple &>(*this), bs, \
                                                                                conn);                           \
    }                                                                                                            \
    template<size_t I>                                                                                           \
