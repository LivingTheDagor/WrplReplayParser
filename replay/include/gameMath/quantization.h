...
#define FVAL_SIGN_BIT(PackType, Bits)         (PackType(1) << ((Bits) - 1))
  ...
  __forceinline T pack_unit_vec(Point3 v, const size_t bits) {
    ...
    if (halfBits >= 32) // Due to forceinline we will have compile-time branches here for the compile-time bits
      return T((pack_scalar_signed<uint64_t>(oct.x, halfBits) << halfBits) |
               pack_scalar_signed<uint64_t>(oct.y, halfBits));
    else
      return T((pack_scalar_signed<uint32_t>(oct.x, halfBits) << halfBits) |
               pack_scalar_signed<uint32_t>(oct.y, halfBits));
  }
  ...
  struct QuantizedQuat {
    ...
    QuantizedQuat(const Quat &q) {
      ...
      valPacked |=
        IntType((mi + 1) & 3) << (3 * Bits); // increment max index to make '0' encode identity quat (0,0,0,1)
      qquat = valPacked;
    }
    Quat unpackQuat() const {
      ...
      for (unsigned i = 0, j = 0; i < 4; ++i)
        if (i != mi) {
          float v =
            FValQuantizer<Bits, InvertedSqrt2Scale>::unpackSigned((qquat >> (j * Bits)) & IntType((1LL << Bits) - 1));
          q[i] = v;
          qv -= sqr(v);
          ++j;
        }
