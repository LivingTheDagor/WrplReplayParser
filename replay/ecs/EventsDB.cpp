...
namespace ecs {
  ...
  bool EventsDB::registerEvent(event_type_t type, event_size_t sz, event_flags_t flags, const char *name,
    ...
    if (sz >= Event::max_event_size || sz < sizeof(Event)) {
      EXCEPTION("Can't register Event <0x%X|%s> of size <%d>, size not in [%d,%d)", type, name, sz, sizeof(Event),
                +Event::max_event_size);
      // return false;
    }
    if ((flags & EVFLG_DESTROY) && (!d || !m)) {
      EXCEPTION("Can't register Event <0x%X|%s>, which requires Destroy, but doesn't provides destroy/move functions",
                type, name);
    ...
    if (!(evCast == EVCAST_UNICAST || evCast == EVCAST_BROADCAST))
      EXCEPTION("Event <0x%X|%s> registered as %s instead of Unicast or Broadcast", type, name,
    ...
    if (id == invalid_event_id) {
      ...
      if (name && eventsInfo.get<EVENT_NAME>()[id] != name) {
        EXCEPTION("Event hash collision found <0x%X|%s> collides with %s", type,
