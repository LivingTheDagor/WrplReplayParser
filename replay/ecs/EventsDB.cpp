...
namespace ecs {
  ...
  EventsDB::EventsDB() {
    for (auto ei = EventInfoLinkedList::tail; ei != nullptr; ei = ei->next) {
      registerEvent(ei->getEventType(), ei->getEventSize(), ei->getEventFlags(), ei->getEventName(),
                    ei->getDestroyFunc(), ei->getMoveOutFunc());
    }
  }

  bool EventsDB::registerEvent(event_type_t type, event_size_t sz, event_flags_t flags, const char *name,
                               destroy_event *d, move_out_event *m) {
    EVENT_LOGD1("Registering Event of name {}", name);
    if (sz >= Event::max_event_size || sz < sizeof(Event)) {
      EXCEPTION("Can't register Event <{:#x}|{}> of size <{}>, size not in [{},{})", type, name, sz, sizeof(Event),
                +Event::max_event_size);
      // return false;
    }
    if ((flags & EVFLG_DESTROY) && (!d || !m)) {
      EXCEPTION("Can't register Event <{:#x}|{}>, which requires Destroy, but doesn't provides destroy/move functions",
                type, name);
      // if (!d) // if there is destroy function, it is possible to work without move. Event just can't be sent to
      // loading entity.
      //   return false;
    }
    if ((d || m) && !(flags & EVFLG_DESTROY))
      EVENT_LOGE("Event <{:#x}|{}> provides destroy/move functions but is trivially destructible", type, name);
    int evCast = flags & EVFLG_CASTMASK;
    if (!(evCast == EVCAST_UNICAST || evCast == EVCAST_BROADCAST))
      EXCEPTION("Event <{:#x}|{}> registered as {} instead of Unicast or Broadcast", type, name,
                EV_CAST_STR_TYPES[evCast]);
    ...
    if (id == invalid_event_id) {
      ...
      eventsMap[type] = id;
      eventsInfo.emplace_back(eastl::move(sz), eastl::move(flags), eastl::move(type),
                              std::string(name ? name : "#UnknownEvent#"));
      ...
      if (name && eventsInfo.get<EVENT_NAME>()[id] != name) {
        EXCEPTION("Event hash collision found <{:#x}|{}> collides with {}", type,
                  eventsInfo.get<EVENT_NAME>()[id].c_str(), name);
        // return false;
      }
      // change info. We don't allow to change name, though.
      if (eventsInfo.get<EVENT_SIZE>()[id] != sz || eventsInfo.get<EVENT_FLAGS>()[id] != flags) {
        EXCEPTION("Event ({:#x}|{}) has changed it size {} -> {} or flags {} -> {}", type,
                  name ? name : eventsInfo.get<EVENT_NAME>()[id].c_str(), eventsInfo.get<EVENT_SIZE>()[id], sz,
                  eventsInfo.get<EVENT_FLAGS>()[id], flags);
      } else
        EVENT_LOGE("event ({}|{:#x}) registered twice", name ? name : eventsInfo.get<EVENT_NAME>()[id].c_str(), type);
  ...
  void EventsDB::dump() const {
    for (const auto &em: eventsMap) {
      ...
      EVENT_LOGI("registered {} {}Event <{}|{:#x}> sz {}",
                 ...
                 eastl::get<EventsDB::EVENT_NAME>(e).c_str(), eastl::get<EventsDB::EVENT_TYPE>(e),
                 eastl::get<EventsDB::EVENT_SIZE>(e));
    }
  }
