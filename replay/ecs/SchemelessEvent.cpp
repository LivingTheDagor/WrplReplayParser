...
namespace ecs {
  ...
  SchemelessEvent::SchemelessEvent(ecs::event_type_t tp, ecs::event_flags_t flags_) :
    ecs::Event(tp, sizeof(SchemelessEvent), flags_) {
...
#endif
    new (allocateAt) SchemelessEvent(static_cast<SchemelessEvent &&>(from));
    // work around, optimization, to prevent further call for destructor when event is moving from main queue to
    // loadingEntities queue destructor in that case WON'T be called, which is technically UB (but it is optimal
    // behaviour, since we know there is nothing to be done in destructor anyway)
