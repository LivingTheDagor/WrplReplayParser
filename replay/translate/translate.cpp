...
public:
...
#if !defined(NDEBUG)
    if (table[1] != this->template get<1>())
      EXCEPTION("tuple_vector layout assumption failed");
#endif
    return table;
  }

  uint32_t get_offset(translate_index_t index, Languages lang) const {
  ...
  void set_offset(translate_index_t index, Languages lang, uint32_t offset) {
    if (lang < Languages::English || lang >= Languages::LENGTH)
      EXCEPTION("invalid language {}", static_cast<int>(lang));
...
private:
  ...
  language_tuple_vec indexToOffs;
...
const char *translate_table_t::get_value(translate_index_t index, Languages lang) {
  auto offs = indexToOffs.get_offset(index, lang);
  if (offs == 0xFFFFFFFF) // no value filled
    return "";
...
const char *translate::localize(std::string_view value, Languages lang, ReturnPolicy return_policy) {
  // does key exist?
...
void translate::set_default_language(Languages lang) {
    ...
    for (int langColNo = 0; langColNo <= max_col; langColNo++) {
      ...
      if (bmap_col[langColNo] < 0 || skip_key)
        ...
        if (l != 0) {
          ...
          this->indexToOffs.set_offset(idx, (Languages) (langColNo + 1), ref);
        }
      }
    }
